...
export async function GET() {
    ...
    if (!trackingCode) {
        return NextResponse.json({ success: false, message: "No tracking code is active at the time!" });

    }
    
    // use IR ip
  const sendRequest = await fetch(`https://tracking.post.ir/search.aspx?id=${trackingCode.code}`,
    {
      headers: {
        accept:
  ...
  const textContentRegex = new RegExp("</?!?(div|span)[^>]*>", "g");
